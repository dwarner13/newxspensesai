...
import { Brain, Sparkles, X, Crown, Zap } from "lucide-react";
...
const AskAI = ({ className = "" }: AskAIProps) => {
  const { user } = useAuth();
  const { hasAccess, userPlan } = useAdminAccess();
  ...
  useEffect(() => {
    const checkSelection = async (e: MouseEvent) => {
  ...
  const askAI = async () => {
    ...
    if (!hasAccess('premium')) {
      setResponse("🔒 Upgrade to Premium to access AI-powered insights and suggestions for your financial data!");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const prompt = `You are a financial AI assistant analyzing highlighted text from a user's transaction data: "${selectionText}"
...
Be concise, helpful, and specific to the highlighted text. Format your response in 3 short paragraphs.`;
      ...
      const aiReply = data.choices?.[0]?.message?.content || "No suggestions available.";
      
      setResponse(aiReply);
      
      // Track AI usage for analytics and award XP
      await supabase
        .from('xp_activities')
        .insert({
          user_id: user?.id,
          ...
          xp_earned: 1,
          description: 'Used AI suggestion feature'
        });

    } catch (error) {
      console.error('AI request error:', error);
      setResponse("AI suggestions are temporarily unavailable. Please try again later.");
  ...
  const handleUpgrade = () => {
    ...
    // In production, this would redirect to a pricing/upgrade page
  };
          ...
          >
            ...
            <p className="text-xs text-gray-500 mt-1">
              {!hasAccess('premium') 
                ...
                : 'Click for AI-powered suggestions'
              }
            </p>
          </motion.div>
        )}
            ...
            <div className="flex justify-between items-start mb-3">
              <div className="flex items-center space-x-2">
                ...
                <strong className="text-gray-900">
                  {!hasAccess('premium') ? 'Premium Feature' : 'AI Suggestion'}
                </strong>
              </div>
              <button 
                onClick={() => setResponse("")}
