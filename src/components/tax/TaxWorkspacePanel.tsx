/**
 * TaxWorkspacePanel Component
 * 
 * Left sidebar panel for Tax Assistant workspace
 * Matches ByteWorkspacePanel structure
 */

import React from 'react';
import { Calculator, FileText, Upload, Calendar } from 'lucide-react';
...
interface StatusCard {
  ...
  description: string;
  timestamp: string;
  badge: 'overview' | 'deductions' | 'documents' | 'deadlines';
...
const statusCards: StatusCard[] = [
  {
    id: '1',
    title: 'Tax Overview',
    description: '$1,180 estimated refund',
  ...
  },
  {
    ...
    description: '$4,250 in deductions',
    timestamp: 'Just now',
    badge: 'deductions',
...
const badgeStyles = {
  overview: 'bg-blue-400/10 text-blue-400 border-blue-400/30',
  deductions: 'bg-green-400/10 text-green-400 border-green-400/30',
  documents: 'bg-purple-400/10 text-purple-400 border-purple-400/30',
  deadlines: 'bg-amber-400/10 text-amber-400 border-amber-400/30',
};
...
export function TaxWorkspacePanel() {
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 h-full flex flex-col">
      ...
      <div className="flex items-center gap-3 mb-3 flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-semibold text-white truncate">TAX WORKSPACE</h3>
          <p className="text-xs text-slate-500">Tax optimization</p>
        </div>
      </div>
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between mb-2">
                ...
                <div className={`px-2 py-1 rounded text-xs font-medium border flex-shrink-0 ml-2 ${badgeStyles.deductions}`}>
                  {badgeLabels.deductions}
                </div>
              </div>
              <p className="text-xs text-slate-400 mt-2">
      ...
      {/* Card 3 - Documents & Deadlines (flex-1 to fill remaining height) */}
        ...
        <div className="p-4 rounded-lg bg-slate-800 border border-slate-700 h-full flex flex-col">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs mb-3 flex-shrink-0">
            <div className="rounded-xl bg-slate-900/60 px-3 py-2 border border-slate-700/50">
              <p className="text-slate-400">W-2s</p>
              <p className="mt-1 text-sm font-semibold text-slate-50">2</p>
            </div>
            <div className="rounded-xl bg-slate-900/60 px-3 py-2 border border-slate-700/50">
              <p className="text-slate-400">1099s</p>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto hide-scrollbar space-y-2 text-[11px] text-slate-200">
            <div className="rounded-lg bg-slate-900/70 px-3 py-2 border border-slate-700/50">
              📄 <span className="font-medium">12 documents</span> processed successfully
            </div>
            <div className="rounded-lg bg-slate-900/70 px-3 py-2 border border-slate-700/50">
              📅 <span className="font-medium">Filing deadline:</span> April 15, 2025
            </div>
            <div className="rounded-lg bg-slate-900/70 px-3 py-2 border border-slate-700/50">
              💰 <span className="font-medium">$1,180</span> estimated refund
            </div>
          </div>
        </div>
      </div>
