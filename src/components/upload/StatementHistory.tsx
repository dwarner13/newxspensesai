...
function issuerColor(issuer: string): string {
  ...
  if (s.includes("triangle mastercard") || s.includes("canadian tire")) return T.accent;
  return T.muted;
}
...
function issuerEmoji(issuer: string): string {
  ...
  if (s.includes("triangle mastercard") || s.includes("canadian tire")) return "🏪";
  return "📁";
}
  ...
  return (
    <div style={{ marginTop: 32 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
        ...
        {!selectMode ? (
          <button onClick={() => { setSelectMode(true); setOpenFolders(new Set(folders.map(f => f.key))); }} style={{ fontSize: 11, fontWeight: 700, color: T.dim, background: "transparent", border: `1px solid ${T.border}`, borderRadius: 6, padding: "3px 10px", cursor: "pointer" }}>Select</button>
        ) : (
          <button onClick={clearSelection} style={{ fontSize: 11, fontWeight: 700, color: T.dim, background: "transparent", border: `1px solid ${T.border}`, borderRadius: 6, padding: "3px 10px", cursor: "pointer" }}>Cancel</button>
        )}
      ...
      {selectMode && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, padding: "10px 14px", borderRadius: 10, background: T.surface, border: `1px solid ${T.border}` }}>
          <span style={{ fontSize: 12, color: T.dim, flex: 1 }}>{selected.size === 0 ? "Tap rows to select" : `${selected.size} selected`}</span>
          <button onClick={selectAllEmpty} style={{ fontSize: 11, fontWeight: 600, color: T.amber, background: `${T.amber}12`, border: `1px solid ${T.amber}30`, borderRadius: 6, padding: "3px 10px", cursor: "pointer" }}>Select empty ({emptyCount})</button>
          <button onClick={selectAll} style={{ fontSize: 11, fontWeight: 600, color: T.dim, background: "transparent", border: `1px solid ${T.border}`, borderRadius: 6, padding: "3px 10px", cursor: "pointer" }}>Select all</button>
          {selected.size > 0 && !confirmBulk && (
            <button onClick={() => setConfirmBulk(true)} style={{ fontSize: 11, fontWeight: 700, color: "#fff", background: T.red, border: "none", borderRadius: 6, padding: "4px 14px", cursor: "pointer" }}>🗑 Delete {selected.size}</button>
          )}
          {confirmBulk && (
            <>
              <span style={{ fontSize: 11, color: T.red, fontWeight: 700 }}>Sure? This cant be undone.</span>
              <button onClick={deleteSelected} disabled={deleting} style={{ fontSize: 11, fontWeight: 700, color: "#fff", background: T.red, border: "none", borderRadius: 6, padding: "4px 14px", cursor: "pointer", opacity: deleting ? 0.5 : 1 }}>{deleting ? "Deleting…" : "Yes, delete"}</button>
              <button onClick={() => setConfirmBulk(false)} style={{ fontSize: 11, fontWeight: 600, color: T.dim, background: "transparent", border: `1px solid ${T.border}`, borderRadius: 6, padding: "3px 8px", cursor: "pointer" }}>Cancel</button>
            </>
          )}
        </div>
      )}
            ...
            <div key={folder.key} style={{ borderRadius: 14, overflow: "hidden", border: `1px solid ${isOpen ? color + "33" : T.border}`, background: T.surface, transition: "border-color 0.2s" }}>
              <div style={{ display: "flex", alignItems: "center", position: "relative" }}>
                <button onClick={() => toggleFolder(folder.key)} style={{ flex: 1, display: "flex", alignItems: "center", gap: 12, padding: "14px 18px", background: "none", border: "none", cursor: "pointer", textAlign: "left" }}>
                  {selectMode && (
                    <div onClick={e => { e.stopPropagation(); folderAllSelected ? folder.imports.forEach(i => setSelected(prev => { const n = new Set(prev); n.delete(i.id); return n; })) : folder.imports.forEach(i => setSelected(prev => new Set(prev).add(i.id))); }} style={{ width: 18, height: 18, borderRadius: 5, border: `2px solid ${folderAllSelected ? T.red : T.border}`, background: folderAllSelected ? T.red : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, cursor: "pointer" }}>
                ...
                {!selectMode && (
                  <div style={{ position: "relative", flexShrink: 0 }}>
                    <button onClick={e => { e.stopPropagation(); setFolderMenu(folderMenu === folder.key ? null : folder.key); }} style={{ padding: "14px 14px 14px 4px", background: "none", border: "none", cursor: "pointer", color: T.dim, fontSize: 16, lineHeight: 1 }}>⋯</button>
                    {folderMenu === folder.key && (
                      <>
                        ...
                        <div style={{ position: "absolute", right: 0, top: "100%", zIndex: 50, minWidth: 200, borderRadius: 10, background: "#0f1a2e", border: `1px solid ${T.border}`, boxShadow: "0 8px 32px rgba(0,0,0,0.4)", overflow: "hidden" }}>
                          <button onClick={() => { folder.imports.forEach(i => setSelected(prev => new Set(prev).add(i.id))); setSelectMode(true); setOpenFolders(new Set([folder.key])); setFolderMenu(null); }} style={{ width: "100%", textAlign: "left", padding: "10px 16px", fontSize: 12, fontWeight: 600, color: T.muted, background: "none", border: "none", borderBottom: `1px solid ${T.border}`, cursor: "pointer" }}>☑ Select all in folder</button>
                          <button onClick={() => { const e2 = folder.imports.filter(i => (!i.committed_count || i.committed_count === 0) && i.status !== "committed"); e2.forEach(i => setSelected(prev => new Set(prev).add(i.id))); setSelectMode(true); setOpenFolders(new Set([folder.key])); setFolderMenu(null); }} style={{ width: "100%", textAlign: "left", padding: "10px 16px", fontSize: 12, fontWeight: 600, color: T.amber, background: "none", border: "none", borderBottom: `1px solid ${T.border}`, cursor: "pointer" }}>⚠ Select empty in folder</button>
                          <button onClick={async () => { setFolderMenu(null); const sb = getSupabase(); if (!sb || !userId) return; for (const imp of folder.imports) { await sb.from("transactions").delete().eq("import_id", imp.id).eq("user_id", userId); await sb.from("imports").delete().eq("id", imp.id).eq("user_id", userId); } setImports(prev => prev.filter(i => !folder.imports.map(f => f.id).includes(i.id))); }} style={{ width: "100%", textAlign: "left", padding: "10px 16px", fontSize: 12, fontWeight: 700, color: T.red, background: "none", border: "none", cursor: "pointer" }}>🗑 Delete entire folder</button>
                        </div>
                      </>
                    )}
                  </div>
              ...
              {isOpen && (
                <div style={{ borderTop: `1px solid ${T.border}` }}>
                  {folder.imports.map((imp, idx) => {
                    ...
                    const meta = imp.document_id ? docMetas.get(imp.document_id) : undefined;
                    const detectedIssuer = detectIssuer(imp, meta);
                    let name: string;
                    if (meta?.statement_period && detectedIssuer !== "Unknown") {
                      name = `${detectedIssuer}${meta.account_last4 ? ` \u00B7\u00B7\u00B7${meta.account_last4}` : ""} \u00B7 ${meta.statement_period}`;
                    } else {
                    ...
                    return (
                      <div key={imp.id} onClick={() => { if (selectMode) { toggleSelect(imp.id); } else { window.location.href = `/dashboard/transactions?import_id=${imp.id}`; } }} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 18px 11px 66px", borderBottom: idx < folder.imports.length - 1 ? `1px solid ${T.border}` : "none", background: isChecked ? `${T.red}10` : "transparent", transition: "background 0.15s", cursor: "pointer" }}>
                        {selectMode && (
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 10, color: T.dim, marginTop: 1 }}>
                            ...
                            {imp.committed_count != null && imp.committed_count > 0 ? ` · ${imp.committed_count} tx` : <span style={{ color: T.red }}> · 0 transactions</span>}
                          </div>
                        </div>
                        ...
                        {isEmpty && !selectMode && <span style={{ fontSize: 10, color: T.red, fontWeight: 700, flexShrink: 0 }}>⚠</span>}
                      </div>
                    );
                  })}
                </div>
