 ...
 * 
 * Tag (Smart Categories) chat slideout panel using PrimeSlideoutShell
...
export interface TagChatPanelProps {
  isOpen: boolean;
  onClose: () => void;
  conversationId?: string;
}

export function TagChatPanel({
  isOpen,
  onClose,
  conversationId,
}: TagChatPanelProps) {
  ...
  const config = EMPLOYEE_CHAT_CONFIG['tag-ai'];
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      console.error('[TagChatPanel] Send failed:', err);
    }
  };
          ...
          <PrimeSlideoutShell
            ...
            footer={
              <div className="space-y-3">
                {TAG_SUGGESTED_PROMPTS.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {TAG_SUGGESTED_PROMPTS.map((prompt) => (
                      <button
                        key={prompt.id}
                        type="button"
                ...
                <form onSubmit={handleSend} className="flex items-center gap-3">
                  <div className="flex-1">
                    <textarea
                      ...
                      className="h-12 w-full resize-none rounded-2xl border border-white/10 bg-[#050816]/80 px-4 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-400/60"
                      rows={1}
                  ...
                  <button
                    ...
                    className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-yellow-300 via-amber-400 to-orange-500 shadow-[0_0_22px_rgba(251,191,36,0.65)] transition-transform duration-150 hover:scale-[1.05] active:scale-[0.97] disabled:opacity-60 disabled:shadow-none"
