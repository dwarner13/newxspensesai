...
import { FileText, DollarSign, GitBranch, Send, CreditCard } from 'lucide-react';
...
export function DebtUnifiedCard({ onExpandClick, onChatInputClick }: DebtUnifiedCardProps) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col h-full">
      <div className="bg-gradient-to-br from-rose-900/40 to-slate-900/10 border-b border-slate-800 pb-6 flex-shrink-0 -mx-6 -mt-6 px-6 pt-6">
        <div className="flex items-start gap-4 mb-3">
          <div className="w-16 h-16 bg-rose-600 rounded-full flex items-center justify-center shadow-lg shadow-rose-500/50 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-slate-400 mt-1 line-clamp-2">
              Debt elimination specialist · Optimize your path to freedom
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-4 mb-3">
          <div className="flex-1 flex flex-col items-center text-center">
            <div className="text-2xl font-bold text-rose-400">$12,450</div>
          ...
          <div className="flex-1 flex flex-col items-center text-center">
            ...
            <div className="text-xs text-slate-500">To Payoff</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Button 
            ...
            className="flex-1 min-w-0 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-rose-500/30 text-white text-xs sm:text-sm"
          >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto -mx-6 px-6 py-4">
        <div className="flex flex-col items-center justify-center h-full text-center">
          ...
          <h3 className="text-xl font-bold text-white mb-2">Welcome to Debt Payoff Planner</h3>
          <p className="text-slate-400 mb-4 max-w-md">
            Strategically eliminate debt and achieve financial freedom. Optimize your payoff plan.
          </p>
        </div>
      </div>
      ...
      <div className="pt-3 pb-0 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-300/80 border-t border-slate-800/50 flex-shrink-0 -mx-6 px-6">
        ...
        <div className="text-[11px] text-slate-400">
          Secure • Always Planning
        </div>
      </div>

      <div className="flex-shrink-0 -mx-6 px-6">
        <div 
          className="flex items-center gap-3 bg-slate-800 rounded-xl px-3 py-2 border border-slate-700 focus-within:border-rose-500 transition-all duration-200 cursor-pointer"
          onClick={onChatInputClick || onExpandClick}
        >
          <input
            type="text"
            placeholder="Ask about your debt payoff plan…"
            value={inputValue}
          ...
          <button
            onClick={handleSend}
            className="w-10 h-10 bg-gradient-to-br from-rose-500 to-rose-600 rounded-full flex items-center justify-center shadow-lg shadow-rose-500/30 hover:scale-105 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
