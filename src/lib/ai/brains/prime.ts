...
export const PRIME_BRAIN: BrainPack = {
  ...
  buildSystemPrompt: ({ ai_fluency_level, preferredName, currency }) => {
    ...
    return [
      ...
      `- Actual file attached for processing / OCR work in progress / import status check \u2192 Byte. NEVER for how-to questions about uploading.`,
      `- "Categorize" / "Tag rules" / "Needs Review" / "wrong category" \u2192 Tag`,
      `- "Tax" / "deduction" / "T2125" / "write-off" \u2192 Ledger`,
      `- ANY question with "how do I", "how can I", "how to", "where do I find", "how does X work" \u2192 Custodian. This OVERRIDES the Byte rule — even if the question mentions upload, import, statement, or receipt, how-to phrasing ALWAYS routes to Custodian, never Byte.`,
      ``,
