  ...
  const generateAIInsights = (): AIInsight[] => {
    ...
    if (totalTransactions > 0) {
      ...
      if (avgPerDay > 2) {
        insights.push({
          ...
          message: `You are processing ${avgPerDay.toFixed(1)} transactions per day on average. Consider batch processing to save time.`,
          priority: 'medium'
        });
      }
    }
    ...
    if (uniqueCategories < 3) {
      ...
      type: 'opportunity',
        ...
        message: `You are currently tracking ${uniqueCategories} spending categories. Consider adding more categories for better financial insights.`,
      priority: 'medium'
      });
...
Generated by XspensesAI - Your Financial Intelligence Companion
  ...
  const generateAnalyticsBotResponse = (message: string): string => {
    ...
    if (lowerMessage.includes('software') || lowerMessage.includes('tools')) {
      ...
      return `💻 Your software spending is $${softwareSpending.toLocaleString()} (${softwarePercentage}% of total). That's ${softwareSpending > 5000 ? 'quite substantial' : 'reasonable'}! I notice you are investing in tools - smart move! 🚀 Want me to suggest ways to optimize these costs or find better deals?`;
    }
    ...
    if (lowerMessage.includes('how') && lowerMessage.includes('doing')) {
      ...
      return `Hey friend! 😊 Let me give you the real talk: Your tax readiness score is ${taxScore}% - that's ${readiness}! 💪 I've processed ${totalTransactions} documents and found $${totalAmount.toLocaleString()} in transactions. You are doing great! 🎉 Want me to help you get that score even higher? I've got some tricks up my sleeve! ✨`;
    }

    // AI asks intelligent follow-up questions
    if (lowerMessage.includes('why') || lowerMessage.includes('what')) {
      return `🤔 Great question! Let me think about this... I'm analyzing your ${totalTransactions} documents and seeing some interesting patterns. ${totalAmount > 10000 ? 'You are moving some serious money! 💰' : 'You are building a solid foundation! 🏗️'} What specific aspect are you most curious about? I can dive deep into categories, vendors, trends, or help you spot opportunities you might be missing! 🔍`;
    }
