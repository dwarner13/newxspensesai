 ...
 * Chime Chat Page
 * 
 * Grade 4 explanation: This page lets you chat with Chime, 
 * one of the AI employees. The chat sends your messages to the server,
 * and Chime responds with helpful information.
 */
export default function ChimeChat() {
  const { user } = useAuth();
  const [input, setInput] = useState("");
  
  // Grade 4 explanation: usePrimeChat hook connects to the chat server
  // and manages messages. We pass 'chime' to tell it which employee to use.
  ...
  useEffect(() => {
    const payload = localStorage.getItem('chime:payload');
    if (payload) {
      try {
        ...
        if (data && typeof data === 'object') {
          ...
          localStorage.removeItem('chime:payload');
        }
      } catch (e) {
        // Ignore parse errors
      }
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      <h1 className="text-xl font-semibold mb-4">Chime</h1>
