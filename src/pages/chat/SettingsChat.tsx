 ...
 * Settings Chat Page
 * 
 * Grade 4 explanation: This page lets you chat with Settings, 
 * one of the AI employees. The chat sends your messages to the server,
 * and Settings responds with helpful information.
 */
export default function SettingsChat() {
  ...
  // and manages messages. We pass 'custodian' to tell it which employee to use.
  ...
  useEffect(() => {
    const payload = localStorage.getItem('custodian:payload');
    if (payload) {
      try {
        ...
        if (data && typeof data === 'object') {
          ...
          localStorage.removeItem('custodian:payload');
        }
      } catch (e) {
        // Ignore parse errors
      }
