 ...
 * Tax Chat Page
 * 
 * Grade 4 explanation: This page lets you chat with Tax, 
 * one of the AI employees. The chat sends your messages to the server,
 * and Tax responds with helpful information.
 */
export default function TaxChat() {
  ...
  // and manages messages. We pass 'ledger' to tell it which employee to use.
  ...
  useEffect(() => {
    const payload = localStorage.getItem('ledger:payload');
    if (payload) {
      try {
        ...
        if (data && typeof data === 'object') {
          ...
          localStorage.removeItem('ledger:payload');
        }
      } catch (e) {
        // Ignore parse errors
      }
