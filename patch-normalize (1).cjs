...
// Normalize to LF for matching, then we'll restore
...
if (!normalized.includes(OLD)) {
  console.error('MATCH FAILED - old string not found');
  process.exit(1);
}

let patched = normalized.replace(OLD, NEW);
if (patched === normalized) {
  console.error('NO CHANGE - replace had no effect');
  process.exit(1);
}
