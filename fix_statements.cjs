...
const T = {
  ...
  text: '#e8ecf4', muted: '#94a3b8', dim: '#64748b',
  accent: '#c8a64e', green: '#34d399', red: '#f87171',
  cyan: '#22d3ee', amber: '#fbbf24',
};

type StatementRow = {
  id: string; filename: string; status: string;
  txn_count: number; earliest: string | null; latest: string | null; uploaded_at: string;
};

function statusColor(s: string) {
  if (s === 'committed') return T.green;
  if (s === 'normalizing' || s === 'parsing' || s === 'parsed') return T.amber;
  if (s === 'failed' || s === 'error') return T.red;
  return T.cyan;
}
function statusLabel(s: string) {
  if (s === 'committed') return 'Committed';
  if (s === 'normalizing') return 'Processing';
...
function statusDot(s: string) {
  if (s === 'committed') return String.fromCharCode(10003);
  if (s === 'failed' || s === 'error') return String.fromCharCode(10005);
  return String.fromCharCode(9711);
}
  ...
  return (
    <div onClick={() => isCommitted && navigate('/dashboard/transactions?importId=' + row.id)}
      style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr auto', padding: '11px 16px',
        ...
        background: i % 2 === 0 ? T.bg : T.surface + '60',
        alignItems: 'center', cursor: isCommitted ? 'pointer' : 'default' }}>
  ...
  return (
    <div style={{ marginTop: 32 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 11, color: T.dim }}>{committed.length} committed \u00b7 {active.length} active</div>
      </div>
...
fs.writeFileSync('src/components/upload/StatementHistory.tsx', newContent, 'utf8');
console.log('StatementHistory rewritten');
